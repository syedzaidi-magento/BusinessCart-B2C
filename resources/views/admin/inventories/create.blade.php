        ...
        <form method="POST" action="{{ route('admin.inventories.store') }}">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="shelf_id" id="shelf_id" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 @error('shelf_id') border-red-500 @enderror">
                        @foreach ($shelves as $shelf)
                            <option value="{{ $shelf->id }}" {{ old('shelf_id') == $shelf->id ? 'selected' : '' }}>{{ $shelf->shelf_name }} ({{ $shelf->warehouse->name }})</option>
                        @endforeach
                    </select>
                    @error('shelf_id')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 @error('quantity') border-red-500 @enderror" value="{{ old('quantity', 0) }}" min="0">
                    @error('quantity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg hover:bg-primary-dark transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">Add Inventory</button>
                </div>
            </div>
        </form>
    </div>
