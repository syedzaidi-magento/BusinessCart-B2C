        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 @error('name') border-red-500 @enderror" value="{{ old('name', $user->name) }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 @error('email') border-red-500 @enderror" value="{{ old('email', $user->email) }}">
                    @error('email')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="checkbox" name="is_admin" id="is_admin" value="1" class="h-5 w-5 text-primary focus:ring-primary border-gray-300 rounded" {{ old('is_admin', $user->is_admin) ? 'checked' : '' }}>
                    @error('is_admin')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
