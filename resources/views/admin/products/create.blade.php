...
@section('content')
        ...
        <form action="{{ route('admin.products.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 @error('type') border-red-500 @enderror" onchange="toggleProductType(this.value)">
                ...
                <div id="quantity-container" class="{{ old('type', 'simple') !== 'simple' ? 'hidden' : '' }}">
                    ...
                    <input type="number" name="quantity" id="quantity" class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 @error('quantity') border-red-500 @enderror" value="{{ old('quantity', 0) }}">
                    @error('quantity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div id="custom-attributes">
                    ...
                    @foreach ($attributeKeys as $key)
                        <div>
                            <label for="custom_attributes[{{ $key->key_name }}]">
                                {{ $key->key_name }} ({{ $key->data_type }}) @if ($key->is_required) * @endif
                            </label>
                            <input type="text" name="custom_attributes[{{ $key->key_name }}]" id="custom_attributes[{{ $key->key_name }}]" value="{{ old("custom_attributes.{$key->key_name}") }}">
                        </div>
                    @endforeach
                </div>                
                        ...
                        @if (old('variations'))
                            @foreach (old('variations') as $index => $variation)
                                <div class="variation flex space-x-2 mb-2">
                                    ...
                                    <button type="button" onclick="removeVariation(this)" class="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 transition-colors duration-200">Remove</button>
                                </div>
                            @endforeach
                        @endif
                    </div>
                ...
                <div id="related-products-container" class="{{ in_array(old('type', 'simple'), ['grouped', 'bundle']) ? '' : 'hidden' }}">
                    ...
                    <div id="related-products">
                        @if (old('related_products'))
                            @foreach (old('related_products') as $index => $relatedId)
                                <div class="related-product flex space-x-2 mb-2">
                                    ...
                                    <button type="button" onclick="removeRelatedProduct(this)" class="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 transition-colors duration-200">Remove</button>
                                </div>
                            @endforeach
                        @endif
                    </div>
    ...
    <script>
        ...
        function toggleProductType(type) {
            ...
            quantityContainer.classList.toggle('hidden', type !== 'simple');
            variationsContainer.classList.toggle('hidden', type !== 'configurable');
            relatedContainer.classList.toggle('hidden', !['grouped', 'bundle'].includes(type));
        }
